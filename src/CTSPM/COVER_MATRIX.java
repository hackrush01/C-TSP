...
class COVER_MATRIX {
    ...
    public int[][] coverArray(float[][] distArray, int noc, int dist, int Nc) {
        int[][] coverArray = new int[noc][noc];

        for (int i = 0; i < noc; i++) {     // For filling coverArray with citites within given distance(iterating distArray) and less than Nc
            int counter = 3;     // Counter from 3, so as to fill 3 closest cities before it
//            System.out.print(i+" "); //Debug Only
            for (int j = 0; j < noc && counter < Nc + 3; j++) {      // Iterating coverArray (actual filling and condition check)
...
//            System.out.println(); // Debug only
    ...
    public int[][] closestCity(float[][] distArray, int noc, int dist, int i, int[] covered,
            int[][] coverArray, int coverMax, ArrayList facilities) {

        int counter = 0;  // Taking three closest city
        int numFac = facilities.size();
...
//                System.out.print(coverdist-1 + "-" + coverdist + "->"); // Debug Only
            for (int j = 0; j < numFac && counter < 3; j++) {  //Filling coverArray only first three places
                int fac = (int) facilities.get(j);
                if (covered[fac + 1] == 0) {
                    if ((distArray[i][fac] <= coverdist) && (distArray[i][fac] > coverdist - 1)) {
                        coverArray[i][counter] = fac;  // Taking city INDEX instead of number
//                        System.out.print(coverArray[i][counter] + "    "); // Debug Only
                        counter++;
                    }
                }
