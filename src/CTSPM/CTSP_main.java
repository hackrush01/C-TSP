...
//        float mutateRate = Float.parseFloat((String) JOptionPane.showInputDialog(null, "Enter the mutation rate", "Mutation Rate", 3, null, null, 0.03));
        ...
        int N = popSize / 10; 
        int gen = 0;
        int numOfFac = (noc*30)/100;// setting the number of facility
        int numOfCust = noc - (numOfFac + 1);//setting the number of customers
        int depot = new Random().nextInt(noc);
        int Nc = 28;

        int[][] coorTable = new COORDINATE().coorArray(noc);
        float[][] distTable = new DIST().distArray(coorTable, noc);

        ArrayList facilities = new POPULATION().findFac(distTable, depot, numOfFac);
        ArrayList customers = new POPULATION().findCust(facilities, depot, numOfCust, noc);
        
        new WRITE().delFile2 ();
//        new WRITE().writeFi le(distTable, noc);
        int[][] coverTable = new COVER_MATRIX().coverArray(distTable, noc, dist, Nc);

        System.out.println("+---------------------Initial Population---------------------+");
        int[][] popTable = new POPULATION().popLHS(depot, facilities, customers, coverTable, popSize, dist, distTable);
        ...
        for (; improveCounter < 100; improveCounter++, gen++) {
            ...
            crossTable = new MAKE_FEASIBLE().makeFeasible(crossTable, coverTable, distTable, popSize, noc, facilities, customers);
            crossTable = new FITNESS().calcFitness(crossTable, distTable, noc, popSize);
...
//            popTable = new FITNESS().calcFitness(_2OptTable, distTable, noc, popSize);
            ...
            int[][] dropAndAddTable = new DROP_AND_ADD().dropAndAdd(_2OptTable, distTable, coverTable, popSize, N, facilities, customers);  // Only N random genes will be local searched
            popTable = new FITNESS().calcFitness(dropAndAddTable, distTable, noc, popSize);

            System.out.println();
            System.out.println("+---------------------Surviving Population---------------------+");
//            int[][] survivorTable = new SUR_SELECT().survivor(popTable, dropAndAddTable);
